...
let () =
  (* Create a world *)
  let world_def = World.World_def.create ~gravity:(Vec2.create 0. (-10.)) () in
  let world_id = World.create (Ctypes.addr world_def) in

  (* Create a ground box *)
  let ground_def = Body.Body_def.create ~position:(Vec2.create 0. (-10.)) () in
  ...
  let body_def =
    Body.Body_def.create ~type_:Body.Body_type.Dynamic ~position:(Vec2.create 0. 4.) ()
  in
  ...
  for _ = 0 to 90 do
    ...
    let x = Body.get_position body_id |> Vec2.x in
    let y = Body.get_position body_id |> Vec2.y in
