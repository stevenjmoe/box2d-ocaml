...
B2_API b2TreeStats b2World_OverlapAABB_wrap(b2WorldId w, b2AABB box,
...
B2_API b2TreeStats b2World_OverlapShape_wrap(b2WorldId w, b2ShapeProxy *proxy,
                                             b2QueryFilter filter,
                                             ocaml_overlap_ptr cb, void *ctx) {
  OcamlCallbackData data = {.callback = cb, .context = ctx};
...
B2_API b2TreeStats b2World_CastRay_wrap(b2WorldId w, b2Vec2 origin,
                                        b2Vec2 translation,
                                        b2QueryFilter filter,
                                        ocaml_cast_ray_ptr cb, void *ctx) {
  OcamlCallbackData data = {.callback = cb, .context = ctx};
  return b2World_CastRay(w, origin, translation, filter, b2_cast_ray_proxy,
                         &data);
}

B2_API b2TreeStats b2World_CastShape_wrap(
    b2WorldId worldId, const b2ShapeProxy *proxy, b2Vec2 translation,
    b2QueryFilter filter, ocaml_cast_ray_ptr cb, void *context) {
  OcamlCallbackData data = {.callback = cb, .context = context};
  return b2World_CastShape(worldId, proxy, translation, filter,
                           b2_cast_ray_proxy, &data);
}

// Collision shims

static inline bool
b2_plane_result_proxy(b2ShapeId id, const b2PlaneResult *result, void *data) {
  OcamlCallbackData *cb_data = (OcamlCallbackData *)data;
  assert(cb_data != NULL && cb_data->callback != NULL);
...
static inline bool b2_custom_filter_proxy(b2ShapeId id1, b2ShapeId id2,
                                          void *data) {
  OcamlCallbackData *cb_data = (OcamlCallbackData *)data;
  assert(cb_data != NULL && cb_data->callback != NULL);
...
static inline bool b2_presolve_proxy(b2ShapeId id1, b2ShapeId id2,
                                     b2Manifold *manifold, void *data) {
  OcamlCallbackData *cb_data = (OcamlCallbackData *)data;
  assert(cb_data != NULL && cb_data->callback != NULL);
...
B2_API void b2World_SetPreSolveCallback_wrap(b2WorldId worldId,
                                             ocaml_presolve_ptr cb,
                                             void *context) {
  OcamlCallbackData data = {.callback = cb, .context = context};
  b2World_SetPreSolveCallback(worldId, b2_presolve_proxy, &data);
}

void draw_solid_polygon_trampoline(b2Transform xf, const b2Vec2 *vertices,
                                   int count, float radius, b2HexColor color,
                                   void *raw_data) {
  OcamlDrawData *data = (OcamlDrawData *)raw_data;
  if (data && data->ocaml_draw_solid_polygon_cb) {
    data->ocaml_draw_solid_polygon_cb(&xf, vertices, count, radius, color,
                                      data->ocaml_context);
  }
}
